...
func TestConfigSource_LoadConfig_ValidYAMLFile(t *testing.T) {
	yamlData := `key1: value1
key2: 42
key3: true`
	tempFile, err := os.CreateTemp("", "valid_yaml_*.yaml")
	if err != nil {
...
func TestConfigSource_LoadConfig_NonexistentFile(t *testing.T) {
	...
	config, err := configSource.LoadConfig("nonexistent_file.yaml")
